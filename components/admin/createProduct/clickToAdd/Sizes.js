...
export default function Sizes({sizes,product,setProduct}){
    ...
    return (
        <div>
            ...
            >
                {
                    noSize? "Click if the product has a size":"Click if product has no size"
                }
            </button>
            {
                sizes?sizes.map((size,i)=>(
                    <div className={styles.clicktoadd} key={i}>
                        ...
                        <input type="number" name="qty" placeholder={noSize?"Product Quantity":"Size Quantity"}
                        min={1}
                        value={size.qty}
                        onChange={(e)=>handleSize(i,e)}
                        />

                    <input type="number" name="price" placeholder={noSize?"Product Price":"Size Price"}
                        min={1}
                        value={size.price}
                        onChange={(e)=>handleSize(i,e)}
                        />
